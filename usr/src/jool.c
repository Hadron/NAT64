...
static int parse_opt(int key, char *str, struct argp_state *state)
{
	...
	case ARGP_TCP_TRANS_TO:
		error = set_general_u64(args, SESSIONDB, TCP_TRANS_TIMEOUT, str, TCP_TRANS, MAX_U32/1000,
				1000);
		break;
	case ARGP_FRAG_TO:
		error = set_general_u64(args, FRAGMENT, FRAGMENT_TIMEOUT, str, FRAGMENT_MIN, MAX_U32/1000,
				1000);
		break;
